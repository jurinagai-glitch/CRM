import "dotenv/config";
import pg from "pg";

const { Pool } = pg;

// Supabase's pooler presents a certificate chain signed by Supabase's own
// private CA ("Supabase Root/Intermediate 2021 CA"), not a publicly trusted
// root, so Node's default trust store cannot verify it. Pinning that CA here
// gives real certificate validation (protects against a MITM'd connection)
// instead of disabling verification outright with rejectUnauthorized: false.
//
// Inlined as a string (rather than read from a sibling .pem file) because the
// production build bundles this file into dist/index.js via esbuild, which
// does not copy non-JS assets alongside it — a file read here would 404 in
// production even though it works in dev.
const SUPABASE_CA = `-----BEGIN CERTIFICATE-----

VCgJ
-----END CERTIFICATE-----
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
`;

export const pool = new Pool({
  connectionString: process.env.DATABASE_URL,
  ssl: { ca: SUPABASE_CA, rejectUnauthorized: true },
});
