...
import { ArrowUpRight, CalendarDays, Check, CheckCircle2, ChevronDown, ChevronRight, CircleAlert, Clock3, EyeOff, ListChecks, Plus, RotateCcw, Sparkles, UsersRound } from "lucide-react";
...
export default function ActionRunway({ onNavigate }: { onNavigate: (screen: ScreenTarget) => void }) {
  ...
  const openActions = useMemo(() => initialActions.filter((item) => !completed.includes(item.id) && !dismissed[item.id]), [completed, dismissed]);
  const dismissedActions = initialActions.filter((item) => dismissed[item.id] && !completed.includes(item.id));
  ...
  const dismiss = (id: string) => {
    ...
    setDismissed((prev) => ({ ...prev, [id]: { reason: DEFAULT_DISMISS_REASON, actor: CURRENT_ACTOR, snoozeUntil: formatDate(snoozeUntil) } }));
  ...
  const restore = (id: string) => { setDismissed((prev) => { const next = { ...prev }; delete next[id]; return next; }); toast.success("アクション一覧に戻しました。"); };
