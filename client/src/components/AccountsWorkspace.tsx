...
type TaskItem = { id: string; description: string; due_date: string | null; assignee: string | null; priority: "高" | "中" | "低"; status: "open" | "done" | "dismissed" };
...
export default function AccountsWorkspace({ onNavigate }: { onNavigate: (screen: ScreenTarget) => void }) {
  ...
  const [detail, setDetail] = useState<{ company: Company; meetings: MeetingNote[]; deals: Deal[]; contacts: Contact[]; proposals: Proposal[]; actions: TaskItem[] } | null>(null);
  ...
  return <section className="screen-page enterprise-workspace account-workspace">
    ...
    <div className="enterprise-shell account-shell">
      ...
      <aside className="context-column">
        ...
        <section className="context-card"><div className="context-heading"><div><span className="eyebrow">CONTACTS</span><h3>担当者</h3></div><UserRound size={16} /></div>{detail.contacts.length === 0 && <p className="context-intro">担当者はまだ登録されていません。</p>}{detail.contacts.map((c) => <div className="integrity-line" key={c.id}><span>{c.name}{c.title ? `（${c.title}）` : ""}</span><button onClick={() => removeContact(c.id)} aria-label="削除"><Trash2 size={13} /></button></div>)}<div style={{ display: "flex", gap: 6, marginTop: 8 }}><input placeholder="氏名" value={newContactName} onChange={(e) => setNewContactName(e.target.value)} style={{ flex: 1, border: "1px solid var(--rule)", borderRadius: 6, padding: 6, fontSize: 12 }} /><input placeholder="役職" value={newContactTitle} onChange={(e) => setNewContactTitle(e.target.value)} style={{ width: 80, border: "1px solid var(--rule)", borderRadius: 6, padding: 6, fontSize: 12 }} /><button className="context-link" onClick={addContact}>追加</button></div></section>
