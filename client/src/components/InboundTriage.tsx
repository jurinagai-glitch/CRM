...
import { ArrowUpRight, Building2, CheckCircle2, ChevronDown, ChevronRight, Copy, EyeOff, Inbox, Mail, Plus, Search, ShieldCheck, Sparkles, UserRound } from "lucide-react";
...
export default function InboundTriage({ onNavigate }: { onNavigate: (screen: ScreenTarget) => void }) {
  ...
  const visibleInquiries = inquiries.filter((item) => !dismissed.includes(item.id));
  ...
  const dismiss = () => {
    ...
    setDismissed((ids) => [...ids, inquiry.id]);
    setSelected(remaining[0]?.id ?? "");
    toast.info("対象外にしました。問い合わせ原文は履歴に残ります。");
  };
