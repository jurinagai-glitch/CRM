...
function CompanyPicker({ selected, onSelect }: { selected: Company | null; onSelect: (c: Company) => void }) {
  ...
  if (selected) {
    return <div className="company-picker-selected" style={{ padding: "8px 16px" }}>
      <span style={{ fontSize: 12, color: "#7b8580" }}>取引先</span>
  ...
  return <div className="company-picker" style={{ padding: "8px 16px" }}>
    ...
    <div className="company-picker-results">
      {results.map((c) => <button key={c.id} className="context-link" style={{ display: "block", width: "100%", textAlign: "left", padding: "6px 4px" }} onClick={() => onSelect(c)}>{c.name}<small style={{ marginLeft: 6, color: "#8a908a" }}>{c.category}</small></button>)}
    </div>
    {query.trim() && results.length === 0 && <div style={{ marginTop: 8 }}>
      <p style={{ fontSize: 11, color: "#8a908a", marginBottom: 6 }}>該当する取引先が見つかりません。</p>
      {!creating ? <button className="context-link" onClick={() => setCreating(true)}><Plus size={13} />「{query}」を新規取引先として作成</button> : <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
        <select value={newCategory} onChange={(e) => setNewCategory(e.target.value)} style={{ border: "1px solid #dedbd2", borderRadius: 6, fontSize: 12 }}>{COMPANY_CATEGORIES.map((c) => <option key={c} value={c}>{c}</option>)}</select>
